  ...
  test('all PHP files have correct namespace structure', async () => {
    ...
    for (const file of phpFiles) {
      ...
      test.step(`Validating ${file}`, async () => {
        ...
        // Check for proper namespace declaration
        if (!file.includes('helpers.php')) {
          expect(content).toMatch(/namespace BestCasinoPortal\\/, `${file} should have BestCasinoPortal namespace`);
        }
        
        // Check for PHP 8.1+ strict mode
        expect(content).toMatch(/declare\(strict_types=1\);/, `${file} should have strict types`);
        
        // Check for proper class structure
        if (!file.includes('helpers.php')) {
          expect(content).toMatch(/(?:class|interface|trait|enum)\s+\w+/, `${file} should contain a class, interface, trait, or enum`);
        }
        
        // Verify no Laravel/Eloquent dependencies
        expect(content).not.toMatch(/use Illuminate\\/, `${file} should not contain Laravel dependencies`);
        expect(content).not.toMatch(/extends Model/, `${file} should not extend Laravel Model`);
        expect(content).not.toMatch(/use App\\/, `${file} should not use App namespace`);
      });
    }
