...
export class DatabaseManager {
  ...
  // Session management
  createSession(args: { name: string; description?: string; projectDir?: string; defaultChannel?: string }): Promise<Session> {
    return new Promise((resolve, reject) => {
      const id = uuidv4();
      const now = new Date().toISOString();
      
      this.db.run(
        `INSERT INTO sessions (id, name, description, project_dir, default_channel, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [id, args.name, args.description || '', args.projectDir || '', args.defaultChannel || '', now, now],
        function(err) {
          if (err) {
            reject(err);
          } else {
  ...
  listSessions(limit: number = 10): Promise<Session[]> {
    return new Promise((resolve, reject) => {
      this.db.all(
