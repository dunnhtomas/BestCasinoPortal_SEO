...
Write-Host ""
Write-Host "Setting up SSH key pair..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Testing initial SSH connection..." -ForegroundColor Cyan
ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no $ServerUser@$ServerIP "echo 'Connection successful'"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "Setting up passwordless authentication..." -ForegroundColor Cyan
    
    # Read the public key
    $publicKeyContent = Get-Content $publicKey -Raw
    
    # Create the authorized_keys setup command
    $setupCommand = "mkdir -p ~/.ssh && echo '$publicKeyContent' >> ~/.ssh/authorized_keys && chmod 700 ~/.ssh && chmod 600 ~/.ssh/authorized_keys && echo 'SSH key setup completed'"
    
    # Execute the setup on the server
    ssh $ServerUser@$ServerIP $setupCommand
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  SUCCESS: SSH public key installed successfully" -ForegroundColor Green
        
        # Step 4: Test passwordless connection
        Write-Host ""
        Write-Host "Testing passwordless SSH..." -ForegroundColor Cyan
        ssh -o BatchMode=yes -o ConnectTimeout=10 $ServerUser@$ServerIP "echo 'Passwordless SSH working!'"
        
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  SUCCESS: Passwordless SSH connection successful!" -ForegroundColor Green
            Write-Host ""
            ...
            Write-Host "  WARNING: Passwordless SSH test failed, but key was installed" -ForegroundColor Yellow
        }
    } else {
        Write-Host "  ERROR: Failed to install SSH key" -ForegroundColor Red
    }
} else {
    Write-Host "  ERROR: Initial SSH connection failed" -ForegroundColor Red
